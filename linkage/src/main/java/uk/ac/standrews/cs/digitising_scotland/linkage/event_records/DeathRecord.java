...
public class DeathRecord extends IndividualRecord {
    ...
    public DeathRecord(final Person person) {
        ...
        if (family != null) { // could refactor this with the **new** bride and groom methods...
            ...
            for (final Person parent : family.getPartners()) {
                if (parent.getGender() == 'M') { // the father
    ...
    public static synchronized java.util.Date parseDate(String date_string) throws ParseException {
        return DOB_DATE_FORMAT.parse(date_string);
    }

    private void processDates(Date birth_date, Date death_date) {
        ...
        if (death_date != null) {
            ...
            if (death_year >= DeathRecord.FIRST_YEAR_DOB_PRESENT) setBirthDate(formatDate(birth_date));
        }
    }
