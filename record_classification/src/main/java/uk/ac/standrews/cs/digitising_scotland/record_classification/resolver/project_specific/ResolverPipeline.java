 ...
 */
public class ResolverPipeline<Threshold, K extends AncestorAble<K>, V extends Value<K, Threshold>, P_Comparator extends Comparator<V>, ValidityCriterion, P_ValidityAssessor extends ValidityAssessor<Set<V>, ValidityCriterion>, LossMetric extends Comparable<LossMetric>, P_LossFunction extends LossFunction<Set<V>, LossMetric>> {
    ...
    public ResolverPipeline(final CachedClassifier<TokenSet, V> cache, final boolean multipleClassifications, final P_Comparator classificationComparator, final P_ValidityAssessor classificationSetValidityAssessor, final P_LossFunction lengthWeightedLossFunction,
                    final Threshold CONFIDENCE_CHOP_LEVEL) {
    ...
    public Set<V> classify(final TokenSet tokenSet, final ValidityCriterion validityCriterion) throws Exception {
    ...
    private Set<V> resolverPipeline(MultiValueMap<K, V> multiValueMap, final ValidityCriterion validityCriterion) throws Exception {
        ...
        final Set<V> best = lFA.getBest(validSets);
        if (best == null) { return new HashSet<V>(); }
        return best;
    }
