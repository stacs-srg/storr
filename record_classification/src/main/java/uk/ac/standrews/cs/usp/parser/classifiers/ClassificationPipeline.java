...
/**
 * Contains the classifiers (that extend {@link AbstractClassifier}) that we desire to use plus
 * a classify method that allows us to classify the record using each.
 * Created by fraserdunlop on 25/04/2014 at 12:37.
 */
public class ClassificationPipeline implements Iterable<AbstractClassifier> {
     ...
     * @param classifierList List of trained {@link AbstractClassifier}
     */
     ...
     * @param classifier must implement {@link AbstractClassifier}. Add this classifier to the pipeline.
     */
     ...
     * @param record {@link Record} to classify.
     * @throws IOException Error reading from disk.
     */
    public void classifyRecord(final Record record) throws IOException {

        for (AbstractClassifier classifier : classifierSet) {
            classifier.classify(record);
            if (exactMatchCheck(record)) { return; }
        }
    }

    /**
     * Loops over all classifications in a records classification set and returns true if the
     * record has been classified successfully by exact match lookup.
     * @param record {@link Record} to check
     * @return true if record has been classified by Exact Match successfully, false if not
     */
    ...
    /**
     * Classifies each record in the with each of the classiifers in the pipeline bucket.
     *
     * @param bucket the bucket to classify
     * @throws IOException IOException Error reading from disk.
     */
