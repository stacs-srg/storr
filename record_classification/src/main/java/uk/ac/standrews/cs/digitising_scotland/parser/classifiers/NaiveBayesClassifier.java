...
import uk.ac.standrews.cs.digitising_scotland.parser.datastructures.vectors.CustomVectorWriter;
...
/**
 * Class has methods for training and classifying using the Mahout Naive Bayes Classifier.
 * The most two important methods are train() and classify().
 * 
 * @author jkc25
 *
 */
public class NaiveBayesClassifier extends AbstractClassifier {
     ...
     * @param vectorFactory This is the vector factory used when creating vectors for each record.
     */
    ...
    /**
     * Writes each vector in the bucket using {@link CustomVectorWriter} vectorWriter.
     * @param bucket {@link Bucket} containing the vectors you want to write.
     * @param vectorWriter {@link CustomVectorWriter} instantiated with output path and types.
     ...
     * {@link CustomVectorWriter} writes out {@link Text}, {@link VectorWritable} pairs into the file specified.
     * @param trainingVectorLocation location to write vectors to.
     * @return {@link Writer} with trainingVectorLocation as it's output path and {@link Text}, {@link VectorWritable} as its types.
    ...
    /**
     * Builds a {@link ClassifierResult} from a result vector and the dictionary of interger/classifications.
     * @param result result vector from the Naive Bayes Classifier.
     * @param dictionary Dictionary containing the mapping of Integers to classifications (labels)
     * @return {@link ClassifierResult} containing the details of the classification
     */
     ...
     * @return StandardNaiveBayesClassifier built from the model stored in "target/naiveBayesModelPath"
    ...
    /**
     * Builds a {@link Dictionary} from a label map. Usually used to read the labelindex written by Mahout {@link BayesUtils}.
     * @param labelMapPath location of the label index written by the mahout trainNaiveByaes job
     * return dictionary {@link Dictionary} containing all label/class mappings
     */
     ...
     * Generates a confidence value from a log likelihood.
     * @param logLikelihood
    ...
    private NaiveBayesModel trainNaiveBayes(final String trainingVectorLocation, final String naiveBayesModelPath) throws Exception {
        ...
        //-i  = training vector location, -el = extract label index, -li- place to store labelindex
    ...
    public Pair<Code, Double> classify(final TokenSet tokenSet) throws IOException {
        ...
        //  double confidence = Math.exp(getClassifier().logLikelihood(classificationID, vector)); THIS WONT WORK - Need to fudge it
